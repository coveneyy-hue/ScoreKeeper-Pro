...
const DEFAULT_FIVE_HUNDRED_NULL_POINTS = 50;
const DEFAULT_FIVE_HUNDRED_MANUAL_ADJUST_POINTS = 50;
...
const Security = {
  /** Crée physiquement les valeurs par défaut dans les paramètres sans écraser une valeur existante, même vide. */
  async ensureDefaults() {
    ...
    const appDefaults = {
      fiveHundredNullPoints: DEFAULT_FIVE_HUNDRED_NULL_POINTS,
      fiveHundredManualAdjustPoints: DEFAULT_FIVE_HUNDRED_MANUAL_ADJUST_POINTS,
    };
    for (const [key, value] of Object.entries(appDefaults)) {
      const existing = await DB.get('settings', key);
      if (!existing) await DB.setSetting(key, value);
    }
  },
  ...
  /** Demande et valide un mot de passe enregistré. Une valeur vide désactive la protection correspondante. */
...
const Screens = {
  ...
  render_five_hundred() {
    ...
    const contractListEl = document.getElementById('fh-contract-list');
    if (contractListEl) contractListEl.innerHTML = UI.fhContractListHtml();
  ...
  async render_stats() {
    ...
    // Conserver exactement la base filtrée utilisée pour les classements.
    // Le clic sur un joueur réutilise ces mêmes parties et contrats afin que
    // la ventilation corresponde au pointage affiché, sans recalcul différent.
    UI._statsPlayerBreakdownContext = {
      records,
      contractRecords: baseContractRecords,
      filters: {
        game: document.getElementById('stats-game-filter')?.selectedOptions?.[0]?.textContent || 'Tous les jeux',
        mode: document.getElementById('stats-mode-filter')?.selectedOptions?.[0]?.textContent || 'Tous les modes',
        period: document.getElementById('stats-period-filter')?.selectedOptions?.[0]?.textContent || 'Depuis toujours',
        gameCount: document.getElementById('stats-game-count-filter')?.selectedOptions?.[0]?.textContent || '20 dernières',
        player: document.getElementById('stats-player-filter')?.selectedOptions?.[0]?.textContent || 'Tous les joueurs',
        team: document.getElementById('stats-team-filter')?.selectedOptions?.[0]?.textContent || 'Toutes les équipes',
      },
    };

    const playerLinkHtml = (name, extraClass = '', scope = 'impact') => {
      const key = Stats.nameKey(name);
      return `<button type="button" class="stats-player-link ${extraClass}" data-player-key="${Utils.esc(key)}" data-stats-scope="${Utils.esc(scope)}" onclick="UI.openStatsPlayerBreakdown(this.dataset.playerKey, this.dataset.statsScope)">${Utils.esc(name)}</button>`;
    };
    ...
    const rowHtml = (x, extra='', trophy=false, clickablePlayer=false) => `<div class="stats-row ${trophy ? 'stats-leader' : ''}"><div class="stats-main"><div class="stats-name-line">${trophy ? '<span class="stats-trophy" title="Meilleur pourcentage de victoires" aria-label="Meilleur pourcentage de victoires">🏆</span>' : ''}${clickablePlayer ? playerLinkHtml(x.name, '', 'wins') : `<strong>${Utils.esc(x.name)}</strong>`}</div>${extra}</div><div>${x.wins}/${x.games}</div><div class="stats-pct">${Stats.pct(x.wins,x.games).toFixed(1)} %</div></div>`;
    ...
    if (impactRankingEl) {
      ...
      if (!rankingAvailable) {
        ...
        impactRankingEl.innerHTML = impactRows.length
          ? `<div class="stats-impact-ranking-note">Classement principal du 500 selon l'impact net des contrats. En cas d'égalité, le % de victoires puis le nombre de parties départagent l'ordre. ${identifiedContracts} contrat(s) avec preneur identifié dans les filtres actuels. Clique sur un prénom pour voir les parties et les contrats qui composent son pointage.</div><div class="stats-impact-ranking-list">${rankingRowsHtml}</div>`
    ...
    if (playersEl) {
      ...
      playersEl.innerHTML = rows.length
        ? rows.map(x => rowHtml(x, '', false, true)).join('')
      ...
      if (!advancedAvailable) {
        ...
        const bidderHtml = bidderRows.length ? `
          <div class="stats-advanced-table stats-advanced-table-5">
            ...
            ${bidderRows.map(x => `<div class="stats-advanced-row"><div>${playerLinkHtml(x.name)}<small>${x.types.size} type(s) différent(s)</small></div><div>${x.contracts}</div><div>${x.success}/${x.failed}</div><div class="stats-pct">${Stats.pct(x.success,x.contracts).toFixed(1)} %</div><div class="${x.netImpact >= 0 ? 'stats-net-positive' : 'stats-net-negative'}">${Utils.signed(x.netImpact)}</div></div>`).join('')}
        ...
        const impactHtml = impactRows.length ? `
          <div class="stats-advanced-table stats-advanced-table-impact">
            ...
            ${impactRows.map(x => `<div class="stats-advanced-row"><div>${playerLinkHtml(x.name)}<small>${x.noBidGames} partie(s) sans prendre un contrat</small></div><div>${x.winsWithSuccess}/${x.wins}</div><div>${x.decisiveWins}</div><div class="stats-pct">${Stats.pct(x.contracts,x.hands).toFixed(1)} %</div></div>`).join('')}
...
const UI = {
  ...
  _fhAdjustHoldTimer: null,
  _fhAdjustHoldInterval: null,
  _fhAdjustLongPress: false,
  _fhAdjustSuppressClick: false,
  _gameTimerInterval: null,
  _statsPlayerBreakdownContext: null,
  ...
  async openFhManualAdjustModal() {
    ...
    const html = `
      ...
      <div class="form-group" style="margin-bottom:10px">
        <label class="form-label">Valeur de l’ajustement</label>
        <input class="generic-delta-input" type="number" id="fh-modal-adjust-value" min="0" step="50" value="${defaultPoints}" placeholder="Points">
      </div>
      <div class="fh-manual-adjust-buttons">
        <button class="btn btn-success fh-manual-adjust-btn"
          onpointerdown="UI.fhAdjustHoldStart(1, event)"
          onpointerup="UI.fhAdjustHoldEnd(event)"
          onpointercancel="UI.fhAdjustHoldEnd(event)"
          onpointerleave="UI.fhAdjustHoldEnd(event)"
          onclick="UI.fhAdjustButtonClick(1, event)" title="Ajouter">+ Ajouter</button>
        <button class="btn btn-danger fh-manual-adjust-btn"
          onpointerdown="UI.fhAdjustHoldStart(-1, event)"
          onpointerup="UI.fhAdjustHoldEnd(event)"
          onpointercancel="UI.fhAdjustHoldEnd(event)"
          onpointerleave="UI.fhAdjustHoldEnd(event)"
          onclick="UI.fhAdjustButtonClick(-1, event)" title="Retirer">− Retirer</button>
      </div>
      <div class="setting-sub" style="margin-top:9px">Appui court : applique la valeur affichée. Appui long : modifie cette valeur par pas de 50 points.</div>
    `;
  ...
  async saveFhNullPointsSetting() {
    ...
    const value = Math.max(0, parseInt(input?.value || String(DEFAULT_FIVE_HUNDRED_NULL_POINTS), 10) || 0);
    await DB.setSetting('fiveHundredNullPoints', value);
    if (input) input.value = value;
    Utils.toast(`Partie nulle : ${value} points configurés`, 'success', 3000);
  },

  async saveFhManualAdjustPointsSetting() {
  ...
  async fhNullDeal() {
    ...
    const configuredPoints = await DB.getSetting('fiveHundredNullPoints', DEFAULT_FIVE_HUNDRED_NULL_POINTS);
  ...
  fhAdjustHoldStart(sign, event) {
    if (event?.button !== undefined && event.button !== 0) return;
    this.fhAdjustHoldEnd();
    this._fhAdjustLongPress = false;
    this._fhAdjustSuppressClick = false;
    this._fhAdjustHoldTimer = setTimeout(() => {
      this._fhAdjustLongPress = true;
      this._fhAdjustSuppressClick = true;
      this.fhAdjustInputBy(sign * 50);
      this._fhAdjustHoldInterval = setInterval(() => this.fhAdjustInputBy(sign * 50), 250);
    }, 550);
  },

  fhAdjustHoldEnd(event) {
    if (this._fhAdjustHoldTimer) clearTimeout(this._fhAdjustHoldTimer);
    if (this._fhAdjustHoldInterval) clearInterval(this._fhAdjustHoldInterval);
    this._fhAdjustHoldTimer = null;
    this._fhAdjustHoldInterval = null;
  },

  fhAdjustButtonClick(sign, event) {
    if (this._fhAdjustSuppressClick || this._fhAdjustLongPress) {
      if (event) event.preventDefault();
      this._fhAdjustSuppressClick = false;
      this._fhAdjustLongPress = false;
      return;
    }
    this.fhManualAdjust(sign);
  },
  ...
  async fhManualAdjust(sign) {
    ...
    const input = document.getElementById('fh-modal-adjust-value') || document.getElementById('fh-adjust-value');
    if (input) input.value = await DB.getSetting('fiveHundredManualAdjustPoints', DEFAULT_FIVE_HUNDRED_MANUAL_ADJUST_POINTS);
    this.closeAppModal();
    Screens.render_five_hundred();
  },
  ...
  openStatsPlayerBreakdown(playerKey, scope = 'impact') {
    const context = UI._statsPlayerBreakdownContext;
    if (!context || !playerKey) return;

    const key = Stats.nameKey(playerKey);
    const impactScope = scope !== 'wins';
    const gameKey = (r) => `${r.sourceGameId || ''}|${r.seriesGameNumber || 1}`;
    const sourceGames = impactScope
      ? (context.records || []).filter(r => r.gameType === 'fiveHundred' && r.mode === 'teams')
      : (context.records || []);
    const playerGames = sourceGames
      .filter(r => (r.players || []).some(name => Stats.nameKey(name) === key))
      .slice()
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());
    const selectedKeys = new Set(playerGames.map(gameKey));
    const playerContracts = (context.contractRecords || [])
      .filter(c => Stats.nameKey(c.player) === key && selectedKeys.has(gameKey(c)))
      .slice()
      .sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime());

    const displayName = playerGames
      .flatMap(r => r.players || [])
      .find(name => Stats.nameKey(name) === key)
      || playerContracts.find(c => Stats.nameKey(c.player) === key)?.player
      || playerKey;

    const wins = playerGames.filter(r => (r.winnerPlayers || []).some(name => Stats.nameKey(name) === key)).length;
    const success = playerContracts.filter(c => c.success).length;
    const failed = playerContracts.length - success;
    const totalImpact = playerContracts.reduce((sum, c) => sum + (Number(c.netImpact) || 0), 0);
    const filterText = [
      context.filters?.game,
      context.filters?.mode,
      context.filters?.period,
      context.filters?.gameCount,
    ].filter(Boolean).join(' · ');

    const gamesByKey = new Map(playerGames.map(g => [gameKey(g), g]));
    const teamForPlayer = (g) => (g?.teams || []).find(t => (t.members || []).some(name => Stats.nameKey(name) === key)) || null;
    const scoreLine = (g) => {
      if (!(g?.teams || []).length) return '';
      return g.teams.map((t, idx) => `${Stats.teamLabel(t.members || [])}: ${Number(g.finalScores?.[idx]) || 0}`).join(' · ');
    };
    const timeOnly = (iso) => {
      if (!iso) return '';
      const d = new Date(iso);
    ...
    const contractRows = playerContracts.map((c, idx) => {
      const g = gamesByKey.get(gameKey(c));
      const net = Number(c.netImpact) || 0;
      const contractPoints = Number(c.contractPoints) || 0;
      const awardedPoints = Number(c.awardedPoints) || 0;
      const pointsText = c.success ? Utils.signed(contractPoints) : Utils.signed(-awardedPoints);
      const wonGame = !!g && (g.winnerPlayers || []).some(name => Stats.nameKey(name) === key);
      const hasWinner = !!g && (g.winnerPlayers || []).length > 0;
      const gameOutcome = hasWinner ? (wonGame ? 'Victoire' : 'Défaite') : 'Sans gagnant';
      const playerTeam = teamForPlayer(g);
      const dateText = c.date ? Utils.formatDate(c.date) : '';
      const detailRows = [
        `<div><span>Partie reliée</span><strong>Partie ${c.seriesGameNumber || g?.seriesGameNumber || 1}</strong></div>`,
        dateText ? `<div><span>Date et heure</span><strong>${Utils.esc(dateText)}</strong></div>` : '',
        timeOnly(c.date) ? `<div><span>Heure</span><strong>${Utils.esc(timeOnly(c.date))}</strong></div>` : '',
        playerTeam ? `<div><span>Équipe</span><strong>${Utils.esc(Stats.teamLabel(playerTeam.members || []))}</strong></div>` : '',
        g ? `<div><span>Résultat de la partie</span><strong class="${wonGame ? 'is-success' : (hasWinner ? 'is-fail' : '')}">${Utils.esc(gameOutcome)}</strong></div>` : '',
        Number.isInteger(c.bidPosition) ? `<div><span>Position de mise</span><strong>${c.bidPosition}${c.openingBidder ? ` · ouverture ${Utils.esc(c.openingBidder)}` : ''}</strong></div>` : '',
        `<div><span>Valeur du contrat</span><strong>${contractPoints} pts</strong></div>`,
        !c.success ? `<div><span>Points concédés</span><strong>${awardedPoints} pts</strong></div>` : '',
      ].filter(Boolean).join('');

      return `<details class="stats-player-breakdown-contract-item" ${idx === 0 ? 'open' : ''}>
        <summary>
          <span class="stats-player-breakdown-contract-name">
            <strong>${Utils.esc(Stats.contractDisplayLabel(c.contract))}</strong>
            <small>${Utils.esc(c.success ? 'Contrat réussi' : 'Contrat perdu')}</small>
          </span>
          <span class="stats-player-breakdown-result ${c.success ? 'is-success' : 'is-fail'}">${c.success ? 'Réussi' : 'Perdu'}</span>
          <strong class="${net >= 0 ? 'stats-net-positive' : 'stats-net-negative'}">${Utils.esc(pointsText)}</strong>
        </summary>
        <div class="stats-player-breakdown-contract-detail">
          ${detailRows}
          ${scoreLine(g) ? `<div class="stats-player-breakdown-contract-score"><span>Score final</span><strong>${Utils.esc(scoreLine(g))}</strong></div>` : ''}
        </div>
      </details>`;
    }).join('');

    const breakdownNote = impactScope
      ? `Cette ventilation affiche directement les contrats de ${displayName} retenus par les filtres du classement, du plus récent au plus ancien. Leur somme correspond à l'impact net utilisé dans le classement principal.`
      : `Cette ventilation affiche directement les contrats de ${displayName} provenant des parties retenues par les filtres. Clique sur un contrat pour voir la partie reliée, l'heure et les détails du résultat.`;
    const impactLabel = impactScope ? 'Impact net' : 'Impact 500';

    const body = `<div class="stats-player-breakdown">
      <div class="stats-player-breakdown-note">${Utils.esc(breakdownNote)}</div>
      ${filterText ? `<div class="stats-player-breakdown-filters">${Utils.esc(filterText)}</div>` : ''}
      <div class="stats-player-breakdown-kpis">
        <div><span>${Utils.esc(impactLabel)}</span><strong class="${totalImpact >= 0 ? 'stats-net-positive' : 'stats-net-negative'}">${Utils.signed(totalImpact)}</strong></div>
        <div><span>Victoires</span><strong>${wins}/${playerGames.length}</strong></div>
        <div><span>Contrats R/P</span><strong>${success}/${failed}</strong></div>
      </div>
      <div class="stats-player-breakdown-help">Clique sur un contrat pour afficher sa partie reliée, sa date et son heure, l'équipe, le résultat de la partie, la position de mise et le score final. Les ajustements manuels et les parties nulles ne sont pas attribués à l'impact net individuel.</div>
      <div class="stats-player-breakdown-contract-list">${contractRows || `<div class="stats-player-breakdown-empty">Aucun contrat pris par ${Utils.esc(displayName)} dans les données correspondant aux filtres actuels.</div>`}</div>
    </div>`;

    this.openAppModal(`${impactScope ? 'Ventilation des contrats' : 'Contrats de'} ${displayName}`, body);
  },

  openStatsInfo(key) {
    const infos = {
      impactRanking: ['Classement par impact net', 'Classement principal du 500 en équipes. Impact net = points produits par les contrats réussis moins les points concédés à l’adversaire lors des contrats perdus. Le trophée va au meilleur impact net. Clique sur le prénom d’un joueur pour voir directement les contrats qui composent exactement ce total. Chaque contrat peut être ouvert pour afficher la partie reliée, la date, l’heure et les détails du résultat.'],
    ...
    <div class="screen" id="screen-settings">
      ...
      <div class="card">
        ...
        <div class="setting-sub" style="margin-bottom:14px">
          Les mots de passe sont conservés dans les paramètres de l’application et inclus dans la sauvegarde JSON complète. Laissez un champ vide pour désactiver la protection correspondante. Sur une installation neuve, ils valent tous le mot de passe maître par défaut.
        </div>
        ...
        <div id="password-settings-editor" style="display:none">
          <div class="form-group">
            ...
            <div class="setting-sub">Exigé avant toute modification des mots de passe. Vide = aucune protection maître.</div>
          </div>
          <div class="form-group">
